...
private:
	...
	// APU and PPU pointers
	PPU* ppu;
	APU* apu;
	
	// Helpers
	u8 Read(u16 Address);
	u8 Write(u16 Address, u8 Value);
	u16 WrapAddress(u16 Old, u16 New);
	void Misfire(u16 Old, u16 New);
	u8 Pop();
	void Push(u8 value);
...
public:	
	...
	void SetAPU(APU* in);
	void SetPPU(PPU* in);
	void SetRAM(u8 addr, u8 value) { RAM[addr] = value; }
	void Tick();
	...
	void Instruction()
	{
		...
		t(0,0x00F0000A,0x00F0000A,0x00F0000A,0x00F0000A,0x00F0000A,0x00F0000A,0x00F0000A,0x00F0000A) { addr=u8(addr+d); d=0; Tick(); }
		t(0,0xFA00F000,0xFA00F000,0xFA00F000,0xFA00F000,0xFA00F000,0xFA00F000,0xFA00F001,0xFA00F000) { addr=u8(addr);   addr+=256*Read(PC++); }
		...
		t(0,0x00000000,0x00000000,0x00000000,0x00000000,0x00000000,0x00000000,0x00001010,0x00000000) { P.V = t & 0x40; P.N = t & 0x80; }
		...
		t(0,0x00000000,0xC8CCC4C8,0x00000000,0x00000100,0x00000000,0x00000000,0x00000000,0x00000000) { t = u8(t - 1); }
		t(0,0xC8CCC1C8,0x00000100,0x00000000,0x00000000,0x00000000,0x00000000,0x00000000,0x00000000) { t = u8(t + 1); }
		...
		t(0,0xAAAAAAAA,0x01000000,0x01000000,0x00000000,0x00000000,0x01000000,0x00000000,0x01000000) { t = u8(~t); }
		...
		t(7,0x01000000,0x01000000,0x01000000,0x00000000,0x01000000,0x01000001,0x01000100,0x01000001) { P.raw = t & ~0x30; }
